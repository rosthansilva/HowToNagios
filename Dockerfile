...
RUN    ls -l /tmp && cd /tmp && \
       ...
       : 'aplicando patches ao Nagios Core:' && \
       echo -n "Substituindo \"<sys\/poll.h>\" com \"<poll.h>\": " && \
       sed -i 's/<sys\/poll.h>/<poll.h>/g' ./include/config.h && \
       echo -n "Patching cgi scripts. Informações em : https://gitlab.alpinelinux.org/alpine/aports/-/issues/12516" && \
       ...
       echo -n "Nagios Instalado - Size: " && \
       du -h -s ${NAGIOS_HOME}

# Compile Nagios Plugins
RUN    echo -e "\n\n ===========================\n Configurando - Nagios Plugins\n ===========================\n" && \
       ...
       egrep -rl "\"sys\/poll.h\"" . | xargs sed -i 's/"sys\/poll.h"/"poll.h"/g' && \
       echo "OK" && \
       echo -n "Patching check_mysql_query. Informações : https://gitlab.alpinelinux.org/alpine/aports/-/issues/12601" && \
...
RUN    echo -e "\n\n =====================\n  Configurando NRPE\n =====================\n" && \
       ...
       # make all && \
       make check_nrpe                                                          && \
       echo "NRPE Compilado successfully: OK" && \
       echo -e "\n\n ===========================\n  Instalação NRPE\n ===========================\n" && \
       # make install && \
       cp src/check_nrpe ${NAGIOS_HOME}/libexec/                                && \
       echo "NRPE instalado successfully: OK" && \
       echo -n "Final Nagios installed size: " && \
       du -h -s ${NAGIOS_HOME}


# Criando configuração SSMTP
...
# Make
RUN chmod +x /usr/local/bin/nagios.sh                 \
            /etc/sv/apache/run                           \
            /etc/sv/nagios/run                           \
            /etc/sv/rsyslog/run                       && \
            ...
            dos2unix /usr/local/bin/nagios.sh      && \
            dos2unix /etc/sv/**/run                   && \
